...
class SysOutput(QC.QObject):
    """Handle standard I/O queue"""

    data_avail = QC.Signal()
...
class WidgetProxy(QC.QObject):
    """Handle Shell widget refresh signal"""

    sig_new_prompt = QC.Signal(str)
    sig_set_readonly = QC.Signal(bool)
    sig_edit = QC.Signal(str, bool)
    sig_wait_input = QC.Signal(str)
...
class InternalShell(PythonShellWidget):
    ...
    focus_changed = QC.Signal()
    sig_oedit = QC.Signal(object, bool, object)
    ...
    def __init__(
        ...
        PythonShellWidget.__init__(
            self, parent, get_conf_path("history_internal.py"), profile
        )
    ...
    def start_interpreter(self, namespace):
        ...
        self.interpreter = Interpreter(namespace, self.exitfunc, SysOutput, WidgetProxy)
    ...
    def external_editor(self, filename, goto=-1):
        ...
        try:
            ...
            program.run_program(editor_path, args)
        except OSError:
