...
from plotpy.utils.path import get_conf_path
from plotpy.widgets.console.mixins import BrowseHistoryMixin, SaveHistoryMixin
...
class ShellBaseWidget(ConsoleBaseWidget, SaveHistoryMixin, BrowseHistoryMixin):
    ...
    sig_keyboard_interrupt = QC.Signal()
    execute = QC.Signal(str)
    append_to_history = QC.Signal(str, str)
    ...
    def __init__(self, parent, history_filename, profile=False, initial_message=None):
        ...
        self.__flushtimer = QC.QTimer(self)
    ...
    def set_font(self, font):
        ...
        cursor.select(QG.QTextCursor.Document)
        charformat = QG.QTextCharFormat()
    ...
    input_buffer = QC.Property("QString", _get_input_buffer, _set_input_buffer)
    ...
    def preprocess_keyevent(self, event):
        ...
        meta = event.modifiers() & QC.Qt.MetaModifier  # meta=ctrl in OSX
        if event.key() == QC.Qt.Key_C and (
            (QC.Qt.MetaModifier | QC.Qt.ControlModifier) & event.modifiers()
        ):
        ...
        if self.new_input_line and (
            len(event.text())
            or event.key()
            in (QC.Qt.Key_Up, QC.Qt.Key_Down, QC.Qt.Key_Left, QC.Qt.Key_Right)
        ):
            self.on_new_line()
    ...
    def postprocess_keyevent(self, event):
        ...
        elif key == QC.Qt.Key_Insert and not shift and not ctrl:
        ...
        elif key == QC.Qt.Key_Space and ctrl:
        ...
        elif (key == QC.Qt.Key_Home) or ((key == QC.Qt.Key_Up) and ctrl):
            self._key_home(shift, ctrl)

        elif (key == QC.Qt.Key_End) or ((key == QC.Qt.Key_Down) and ctrl):
        ...
        elif key in (QC.Qt.Key_PageUp, QC.Qt.Key_PageDown):
        ...
        elif key == QC.Qt.Key_Escape and shift:
            self.clear_line()

        elif key == QC.Qt.Key_Escape:
            self._key_escape()

        elif key == QC.Qt.Key_L and ctrl:
            self.clear_terminal()

        elif key == QC.Qt.Key_V and ctrl:
            self.paste()

        elif key == QC.Qt.Key_X and ctrl:
            self.cut()

        elif key == QC.Qt.Key_Z and ctrl:
            self.undo()

        elif key == QC.Qt.Key_Y and ctrl:
            self.redo()

        elif key == QC.Qt.Key_A and ctrl:
            self.selectAll()

        elif key == QC.Qt.Key_Question and not self.has_selected_text():
            self._key_question(text)

        elif key == QC.Qt.Key_ParenLeft and not self.has_selected_text():
            self._key_parenleft(text)

        elif key == QC.Qt.Key_Period and not self.has_selected_text():
