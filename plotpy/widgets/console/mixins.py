...
from plotpy.widgets.console.arraybuilder import NumpyArrayDialog
...
class BaseEditMixin(object):
    ...
    def show_calltip(
        ...
        if at_line is not None:
            cx = 5
            cursor = QG.QTextCursor(self.document().findBlockByNumber(at_line - 1))
            cy = self.cursorRect(cursor).top()
        point = self.mapToGlobal(QC.QPoint(cx, cy))
        ...
        if signature:
            ...
            QW.QToolTip.showText(point, tiptext)
    ...
    def get_position(self, subject):
        ...
        if subject == "cursor":
            ...
            cursor.movePosition(QG.QTextCursor.StartOfBlock)
        elif subject == "eol":
            cursor.movePosition(QG.QTextCursor.EndOfBlock)
        elif subject == "eof":
    ...
    def move_cursor(self, chars=0):
        ...
        direction = QG.QTextCursor.Right if chars > 0 else QG.QTextCursor.Left
        for _i in range(abs(chars)):
            self.moveCursor(direction, QG.QTextCursor.MoveAnchor)
    ...
    def __move_cursor_anchor(self, what, direction, move_mode):
        ...
        if what == "character":
            if direction == "left":
                self.moveCursor(QG.QTextCursor.PreviousCharacter, move_mode)
            elif direction == "right":
                self.moveCursor(QG.QTextCursor.NextCharacter, move_mode)
        elif what == "word":
            if direction == "left":
                self.moveCursor(QG.QTextCursor.PreviousWord, move_mode)
            elif direction == "right":
                self.moveCursor(QG.QTextCursor.NextWord, move_mode)
        elif what == "line":
            if direction == "down":
                self.moveCursor(QG.QTextCursor.NextBlock, move_mode)
            elif direction == "up":
                self.moveCursor(QG.QTextCursor.PreviousBlock, move_mode)
    ...
    def move_cursor_to_next(self, what="word", direction="left"):
        ...
        """
        self.__move_cursor_anchor(what, direction, QG.QTextCursor.MoveAnchor)
    ...
    def extend_selection_to_next(self, what="word", direction="left"):
        ...
        toward *direction* ('left' or 'right')
        """
        self.__move_cursor_anchor(what, direction, QG.QTextCursor.KeepAnchor)
    ...
    def __select_text(self, position_from, position_to):
        ...
        cursor.setPosition(position_to, QG.QTextCursor.KeepAnchor)
        return cursor
    ...
    def get_character(self, position, offset=0):
        ...
        if position < cursor.position():
            cursor.setPosition(position)
            cursor.movePosition(QG.QTextCursor.Right, QG.QTextCursor.KeepAnchor)
    ...
    def get_current_word(self):
        ...
        if cursor.hasSelection():
            ...
            def is_space(move):
                ...
                curs.movePosition(move, QG.QTextCursor.KeepAnchor)
                return not str(curs.selectedText()).strip()

            if is_space(QG.QTextCursor.NextCharacter):
                if is_space(QG.QTextCursor.PreviousCharacter):
                    return
                cursor.movePosition(QG.QTextCursor.WordLeft)

        cursor.select(QG.QTextCursor.WordUnderCursor)
        text = str(cursor.selectedText())
    ...
    def get_current_line(self):
        ...
        cursor = self.textCursor()
        cursor.select(QG.QTextCursor.BlockUnderCursor)
        return str(cursor.selectedText())
    ...
    def find_multiline_pattern(self, regexp, cursor, findflag):
        ...
        try:
            ...
            return
        if findflag & QG.QTextDocument.FindBackward:
            # Find backward
        ...
        if match:
            ...
            fcursor.setPosition(pos2, QG.QTextCursor.KeepAnchor)
            return fcursor
    ...
    def find_text(
        ...
        """Find text"""
        cursor = self.textCursor()
        findflag = QG.QTextDocument.FindFlag()

        if not forward:
            findflag = findflag | QG.QTextDocument.FindBackward

        if case:
            findflag = findflag | QG.QTextDocument.FindCaseSensitively

        moves = [QG.QTextCursor.NoMove]
        if forward:
            moves += [QG.QTextCursor.NextWord, QG.QTextCursor.Start]
            if changed:
                if str(cursor.selectedText()):
                    new_position = min([cursor.selectionStart(), cursor.selectionEnd()])
                    cursor.setPosition(new_position)
                else:
                    cursor.movePosition(QG.QTextCursor.PreviousWord)
        else:
            moves += [QG.QTextCursor.End]
        ...
        pattern = QC.QRegularExpression("\\b{}\\b".format(text) if words else text)
        if case:
            pattern.setPatternOptions(QC.QRegularExpression.CaseInsensitiveOption)
    ...
    def mouseMoveEvent(self, event):
        ...
        if get_error_match(text):
            if not self.__cursor_changed:
                QC.QApplication.setOverrideCursor(QC.QCursor(QC.Qt.PointingHandCursor))
                self.__cursor_changed = True
            event.accept()
            return
        if self.__cursor_changed:
            QC.QApplication.restoreOverrideCursor()
            self.__cursor_changed = False
