...
class CallTipWidget(QW.QLabel):
    ...
    def __init__(self, text_edit, hide_timer_on=False):
        ...
        text edit widget.
        """
        assert isinstance(text_edit, (QW.QTextEdit, QW.QPlainTextEdit))
        super(CallTipWidget, self).__init__(None, QC.Qt.ToolTip)
        self.app = QC.QCoreApplication.instance()

        self.hide_timer_on = hide_timer_on
        self.tip = None
        self._hide_timer = QC.QBasicTimer()
        self._text_edit = text_edit

        self.setFont(text_edit.document().defaultFont())
        self.setForegroundRole(QG.QPalette.ToolTipText)
        self.setBackgroundRole(QG.QPalette.ToolTipBase)
        self.setPalette(QW.QToolTip.palette())
        ...
        self.setMargin(
            1
            + self.style().pixelMetric(QW.QStyle.PM_ToolTipLabelFrameWidth, None, self)
        )
    ...
    def eventFilter(self, obj, event):
        ...
        if obj == self._text_edit:
            ...
            elif etype == QC.QEvent.FocusOut:
                self.hide()

            elif etype == QC.QEvent.Enter:
                if (
                    self._hide_timer.isActive()
                    and self.app.topLevelAt(QG.QCursor.pos()) == self
                ):
                    self._hide_timer.stop()

            elif etype == QC.QEvent.Leave:
    ...
    def paintEvent(self, event):
        ...
        painter.drawPrimitive(QW.QStyle.PE_PanelTipLabel, option)
        painter.end()
