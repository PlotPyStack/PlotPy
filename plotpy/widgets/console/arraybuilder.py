...
class NumpyArrayInline(QW.QLineEdit):
    """ """
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() in [QC.Qt.Key_Enter, QC.Qt.Key_Return]:
            ...
            QW.QLineEdit.keyPressEvent(self, event)
    ...
    def event(self, event):
        ...
        if event.type() == QC.QEvent.KeyPress:
            if event.key() == QC.Qt.Key_Tab or event.key() == QC.Qt.Key_Space:
                text = self.text()
...
class NumpyArrayTable(QW.QTableWidget):
    """ """
    ...
    def keyPressEvent(self, event):
        ...
        """
        if event.key() in [QC.Qt.Key_Enter, QC.Qt.Key_Return]:
            QW.QTableWidget.keyPressEvent(self, event)
    ...
    def reset_headers(self):
        ...
        for r in range(rows):
            self.setVerticalHeaderItem(r, QW.QTableWidgetItem(str(r)))
        for c in range(cols):
            self.setHorizontalHeaderItem(c, QW.QTableWidgetItem(str(c)))
...
class NumpyArrayDialog(QW.QDialog):
    """ """
    ...
    def __init__(self, parent=None, inline=True, offset=0, force_float=False):
        ...
        # Widgets
        self._button_warning = QW.QToolButton()
        ...
        # widget setup
        self.setWindowFlags(QC.Qt.Window | QC.Qt.Dialog | QC.Qt.FramelessWindowHint)
        self.setModal(True)
        ...
        self._layout.addWidget(self._button_warning, 1, QC.Qt.AlignTop)
        self._layout.addWidget(self._button_help, 1, QC.Qt.AlignTop)
    ...
    def event(self, event):
        ...
        """
        if event.type() == QC.QEvent.KeyPress and event.key() == QC.Qt.Key_Tab:
            return False
        return QW.QWidget.event(self, event)
    ...
    def update_warning(self):
        ...
        if not self.is_valid():
            ...
            QW.QToolTip.showText(self._widget.mapToGlobal(QC.QPoint(0, 5)), tip)
        else:
