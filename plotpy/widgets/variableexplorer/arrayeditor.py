...
class ArrayModel(QC.QAbstractTableModel):
    ...
    def rowCount(self, qindex=QC.QModelIndex()):
        """Array row number"""
    ...
    def data(self, index, role=QC.Qt.DisplayRole):
        ...
        if role == QC.Qt.DisplayRole:
            ...
            role == QC.Qt.BackgroundColorRole
            ...
            try:
                ...
                color = QG.QColor.fromHsvF(hue, self.sat, self.val, self.alp)
                return color
            except TypeError:
                return None
        ...
        if dtype == "bool":
            ...
            try:
                ...
                QW.QMessageBox.critical(
                    self.dialog, "Error", "Value error: %s" % str(e)
                )
                return False
        try:
            ...
            QW.QMessageBox.critical(self.dialog, "Error", "Overflow error: %s" % str(e))
            return False
    ...
    def headerData(self, section, orientation, role=QC.Qt.DisplayRole):
        ...
        if role != QC.Qt.DisplayRole:
            return None
        labels = self.xlabels if orientation == QC.Qt.Horizontal else self.ylabels
        if labels is None:
            return int(section)
        else:
...
class ArrayDelegate(QW.QItemDelegate):
    ...
    def createEditor(self, parent, option, index):
        ...
        if model._data.dtype.name == "bool":
            ...
            editor.setAlignment(QC.Qt.AlignCenter)
            if is_number(self.dtype):
                validator = QG.QDoubleValidator(editor)
    ...
    def setEditorData(self, editor, index):
        ...
        text = index.model().data(index, QC.Qt.DisplayRole)
        editor.setText(text)
...
class ArrayView(QW.QTableView):
    ...
    def load_more_data(self, value, rows=False, columns=False):
        ...
        if old_rows_loaded is not None or old_cols_loaded is not None:
            ...
            new_selection = QC.QItemSelection()
            for part in old_selection:
                ...
                part = QC.QItemSelectionRange(top_left, bottom_right)
...
class ArrayEditorWidget(QW.QWidget):
    """ """
    ...
    def __init__(self, parent, data, readonly=False, xlabels=None, ylabels=None):
        ...
        btn_layout.setAlignment(QC.Qt.AlignLeft)
...
class ArrayEditor(QW.QDialog):
    ...
    def setup_and_check(
        ...
        if is_record_array or is_masked_array or data.ndim == 3:
            if is_record_array:
                btn_layout.addWidget(QW.QLabel(_("Record array fields:")))
                names = []
                for name in data.dtype.names:
            ...
            if data.ndim == 3:
                # QSpinBox
                self.index_spin = QW.QSpinBox(self, keyboardTracking=False)
                self.index_spin.valueChanged.connect(self.change_active_widget)
                # QComboBox
                names = [str(i) for i in range(3)]
                ra_combo = QW.QComboBox(self)
                ra_combo.addItems(names)
                ra_combo.currentIndexChanged.connect(self.current_dim_changed)
                # Adding the widgets to layout
                label = QW.QLabel(_("Axis:"))
                btn_layout.addWidget(label)
                btn_layout.addWidget(ra_combo)
                self.shape_label = QW.QLabel()
                btn_layout.addWidget(self.shape_label)
                label = QW.QLabel(_("Index:"))
                btn_layout.addWidget(label)
                btn_layout.addWidget(self.index_spin)
                self.slicing_label = QW.QLabel()
                ...
                btn_layout.addWidget(ra_combo)
            if is_masked_array:
                label = QW.QLabel(_("<u>Warning</u>: changes are applied separately"))
                label.setToolTip(
                    _(
    ...
    def error(self, message):
        ...
        QW.QMessageBox.critical(self, _("Array editor"), message)
        self.setAttribute(QC.Qt.WA_DeleteOnClose)
        self.reject()
