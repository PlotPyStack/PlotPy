...
def test_dataframemodel_sort_is_stable():  # cf. issue 3010
    ...
    assert col2 == [
        str(x) for x in [1, 3, 4, 6, 11, 12, 15, 17, 2, 5, 7, 8, 9, 10, 13, 14, 16]
    ]
...
def test_dataframemodel_set_data_overflow(monkeypatch):
    ...
    for idx, int_type, bit_exponent in test_parameters:
        ...
        try:
            ...
            assert numpy.sum(test_df[0].as_matrix() == model.df.as_matrix()) == len(
                test_df
            )
...
def test_dataframeeditor_edit_overflow(qtbot, monkeypatch):
    ...
    for idx, int_type, bit_exponet in test_parameters:
        ...
        qtbot.wait(500)
        try:
            assert numpy.sum(expected_df[0].values == dialog.get_value().values) == len(
                expected_df
            )
        except AttributeError:
            assert numpy.sum(
                expected_df[0].as_matrix() == dialog.get_value().as_matrix()
            ) == len(expected_df)
...
def test_dataframeeditor_edit_complex(qtbot, monkeypatch):
    ...
    for count, complex_type in test_params:
        ...
        qtbot.wait(500)
        try:
            assert numpy.sum(test_df[0].values == dialog.get_value().values) == len(
                test_df
            )
        except AttributeError:
            assert numpy.sum(
                test_df[0].as_matrix() == dialog.get_value().as_matrix()
            ) == len(test_df)
...
def test_dataframemodel_set_data_bool(monkeypatch):
    ...
    for bool_type in test_params:
        ...
        try:
            assert numpy.sum(expected_df[0].values == model.df.values[:, 0]) == len(
                expected_df
            )
        except AttributeError:
            assert numpy.sum(
                expected_df[0].as_matrix() == model.df.as_matrix()[:, 0]
            ) == len(expected_df)
...
def test_dataframeeditor_edit_bool(qtbot, monkeypatch):
    ...
    for bool_type in test_params:
        ...
        try:
            assert numpy.sum(
                expected_df[0].values == dialog.get_value().values[:, 0]
            ) == len(expected_df)
        except AttributeError:
            assert numpy.sum(
                expected_df[0].as_matrix() == dialog.get_value().as_matrix()[:, 0]
            ) == len(expected_df)
...
def test_no_convert_strings_to_unicode():
    ...
    """
    df = read_csv(
        os.path.join(FILES_PATH, "issue_5833.csv"), index_col=0, encoding="koi8_r"
    )
    dfm = DataFrameModel(df)
