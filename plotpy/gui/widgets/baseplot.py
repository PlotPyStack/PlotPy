...
class BasePlot(QwtPlot):
    ...
    def _resize_selected_items_together(self, item, zoom_dx, zoom_dy):
        ...
        for selitem in self.get_selected_items():
            if (selitem is not item and selitem.can_resize() and
                    IBaseImageItem in selitem.__implements__):
    ...
    def _rotate_selected_items_together(self, item, angle):
        ...
        for selitem in self.get_selected_items():
            if (selitem is not item and selitem.can_rotate() and
                    IBaseImageItem in selitem.__implements__):
    ...
    def get_plot_names(self):
        ...
        """
        return [item.get_filename() for item in
                self.get_items()
                if item.selected]
