...
class SelectTool(InteractiveTool):
    ...
    def setup_filter(self, baseplot):
        ...
        filter.add_event(start_state,
                         KeyEventMatch((Qt.Key_Left, Qt.Key_Right,
                                        Qt.Key_Up, Qt.Key_Down)),
                         self.move_with_arrow, start_state)
        filter.add_event(start_state,
                         KeyEventMatch([(Qt.Key_Left, Qt.ControlModifier),
                                        (Qt.Key_Right, Qt.ControlModifier),
                                        (Qt.Key_Up, Qt.ControlModifier),
                                        (Qt.Key_Down, Qt.ControlModifier)]),
                         self.move_with_arrow, start_state)
        filter.add_event(start_state,
                         KeyEventMatch([(Qt.Key_Left, Qt.ShiftModifier),
                         ...
                         self.rotate_with_arrow, start_state)
...
class RotationCenterTool(InteractiveTool):
    ...
    def __init__(self, manager,
                 ...
                 on_all_items=True):
        super(RotationCenterTool, self).__init__(manager, toolbar_id,
...
#            self.mouse_press(filter, QEvent(QEvent.MouseButtonPress))
    ...
    def mouse_press(self, filter, event):
        ...
        if self.rotation_center :
            ...
            pos = event.pos()
            plot = filter.plot
            self.pos = QPointF(plot.invTransform(2, pos.x()),
                               plot.invTransform(0, pos.y()))
            if not self.on_all_items:
                for item in selected_items:
                    if isinstance(item, TrImageItem):
                        item.set_rotation_point(self.pos.x(), self.pos.y(),
...
class ImageStatsRectangle(AnnotatedRectangle):
    ...
    def __init__(self, x1=0, y1=0, x2=0, y2=0, annotationparam=None,
                 show_surface=False, show_integral=False):
    ...
    def get_infos(self):
        ...
        if len(items) >= 1:
            sorted_items = [it for it in sorted(items, key=lambda obj: obj.z())\
                              if it.isVisible()]
            ...
            return _("No available data")
        return self.image_item.get_stats(*self.get_rect(), self.show_surface,
                                         self.show_integral)
...
class ImageStatsTool(RectangularShapeTool):
    ...
    def create_shape(self):
        ...
        :return:
        """
        return ImageStatsRectangle(0, 0, 1, 1,
                                   show_surface=self.show_surface,
                                   show_integral=self.show_integral), 0, 2
...
class DoAutoscaleTool(CommandTool):
    ...
    def __init__(self,
        ...
        super(DoAutoscaleTool, self).__init__(manager,
                                              title=title,
                                              icon=icon,
                                              tip=tip,
                                              toolbar_id=toolbar_id)
...
def save_snapshot(plot, p0, p1, new_size=None):
    ...
    try:
        ...
        text = _("There is not enough memory left to process "
...
class LockTrImageTool(ToggleTool):
    ...
    def __init__(self, manager, toolbar_id=None):
        super(LockTrImageTool, self).__init__(manager,
                                              title=_("Lock"),
                                              icon=get_icon('lock.png'),
                                              toolbar_id=None)
    ...
    def get_supported_items(self, plot):
        return [_it for _it in plot.get_selected_items() \
                   if isinstance(_it, TrImageItem)]
...
class RectangularSelectionTool(RectangularActionTool):
    ...
    def __init__(self, manager, intersect=True, toolbar_id=DefaultToolbarID):
        super(RectangularSelectionTool, self).__init__(manager, 
                                           self.select_items,
                                           toolbar_id=toolbar_id,
    ...
    def select_items(self, plot, p0, p1):
        ...
        items = get_items_in_rectangle(plot,
                                       p0,
                                       p1,
