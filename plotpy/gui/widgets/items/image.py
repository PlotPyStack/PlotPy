...
class BaseImageItem(QwtPlotItem):
    ...
    def get_stats(self, x0, y0, x1, y1, show_surface=False, show_integral=False):
        ...
        if show_surface and xunit==yunit and xunit is not None and show_integral and zunit is not None:
            if surface != 0:
...
class TransformImageMixin:
    ...
    def move_local_point_to(self, handle, pos, ctrl=None):
        ...
        if self.can_resize():
            ...
            dx = float(zoom * dx)
            dy = float(zoom * dy)
            self.rotation_point[0] = center[0] + (self.rotation_point[0] - center[0]) * zoom
            self.rotation_point[1] = center[1] + (self.rotation_point[1] - center[1]) * zoom
            if self.plot():
    ...
    def resize_with_selection(self, zoom_dx, zoom_dy):
        ...
        center = handles.sum(axis=1) / 4
        if self.rotation_point_move_with_shape:
            self.rotation_point[0] = center[0] + (self.rotation_point[0] - center[0]) * zoom_dx
            self.rotation_point[1] = center[1] + (self.rotation_point[1] - center[1]) * zoom_dy
...
class TrImageItem(TransformImageMixin, RawImageItem):
    ...
    def set_rotation_point_to_center(self):
        handles = self.itr * self.points
        center = handles.sum(axis=1) / 4
        self.set_rotation_point(center[0], center[1],
    ...
    def draw_image(self, painter, canvasRect, src_rect, dst_rect, xMap, yMap):
        ...
        if self.can_rotate():
            ...
            center = QPointF(*axes_to_canvas(self, self.rotation_point[0],
                                             self.rotation_point[1]))
            painter.drawEllipse(center, self.ROTATION_POINT_DIAMETER / 2.,
                                self.ROTATION_POINT_DIAMETER / 2.)
        ...
        if apply_interpolation:
            if force_interp_mode is not None:
                if force_interp_mode in (INTERP_NEAREST, INTERP_LINEAR):
                    ...
                    aa = np.ones((force_interp_size, force_interp_size), self.data.dtype)
